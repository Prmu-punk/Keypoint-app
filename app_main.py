        ...
        try:
            ...
            for i, obj_idx in enumerate(obj_indices):
                ...
                for frame_idx in range(len(tracks)):
                    if frame_idx < len(visibility) and visibility[frame_idx] > 0.5:  # 可见性阈值
    ...
    def save_final_optimized_parameters(self):
        ...
        try:
            ...
            final_object_params = {
                'poses': {},  # R_final
                'centers': {},  # t_final
                'scale': float(self.scale_var.get())  # 添加用户设置的scale参数
            }
            
            # 整合所有段的参数
            for segment_params in self.temp_optimized_params:
            ...
            saved_files = transform_and_save_parameters(
                human_params_dict=final_human_params,
                org_params=self.object_poses,  # 使用self.object_poses作为org_params
                camera_params=self.output,     # 使用self.output作为camera_params
                output_dir=save_dir,
