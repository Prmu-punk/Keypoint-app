...
def rotate(vertices,orient_path,obj_pcd,scale,dist):
    ...
    #pelvis=smpl_vertices[:,0,:]
    print('pelvis',pelvis)
    
    
    m=axis_angle_to_matrix(global_orient_org)
    '''
    euler=matrix_to_euler_angles(m,['X','Y','Z'])
    euler_trans=euler.copy()
    euler_trans[0,[0,2]]=0
    euler_trans[0,1]*=-1
    euler2=matrix_to_euler_angles(transform_matrix,['X','Y','Z'])
    transform_matrix=euler_angles_to_matrix(euler_trans,['X','Y','Z'])
    print(matrix_to_euler_angles(transform_matrix,['X','Y','Z']))
    '''
